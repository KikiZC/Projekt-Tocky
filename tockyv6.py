...
points = [
    ...
    {"position": pos, "distance": 200, "color": (1, 100, 32), "angular_speed": 210, "parent_index": 0, "start_angle": math.radians(0)},
    {"position": pos, "distance": 100, "color": (1, 100, 32), "angular_speed": -100, "parent_index": 1, "start_angle": math.radians(0)},
]
...
def get_button_color():
    return (255, 0, 0) if simulation_running else (0, 100, 0)
...
sbutton_x = width - 110
sbutton_y = 160
sbutton_font = pygame.font.SysFont("Monocraft", 16)

def get_sbutton_color():
    return (200, 0, 0) if stopa else (0, 100, 0)

sbutton_rect = pygame.Rect(sbutton_x, sbutton_y, button_width, button_height)
...
# Vedle tlačítka casu
drvalue_x = dbutton_minus_x + button_width + 10
drvalue_y = dbutton_minus_y
drvalue_font = pygame.font.SysFont("Monocraft", 16)
drvalue_rect = pygame.Rect(drvalue_x, drvalue_y, 60, button_height)
...
while running:
        ...
        if event.type == pygame.QUIT:
            ...
            if event.button == 1:  # Levé tlačítko myši
                ...
                if ibutton_minus_rect.collidepoint(event.pos):
                    ...
                    if index <= 1:
                        index = 1
                if dbutton_plus_rect.collidepoint(event.pos):
                    points[index]["distance"] += 1
                if dbutton_minus_rect.collidepoint(event.pos):
                    points[index]["distance"] -= 1
                    if points[index]["distance"] <= 0:
                        points[index]["distance"] = 0
                if sbutton_rect.collidepoint(event.pos):
                    stopa = not stopa

    r1r = str(points[index]["angular_speed"])
    d1r = str(points[index]["distance"])
    indexr = str(index)
    ...
    # Vykreslení tlačítka
    zaklad_button_color = (0, 100, 0)

    button_text = button_font.render(get_button_text(), True, (255, 255, 255))
    dvalue_text = irvalue_font.render("Vzdálenost: " + d1r, True, (255, 255, 255))
    ...
    sbutton_text = button_font.render("Malování", True, (255, 255, 255))
    ...
    dbutton_plus_rect = pygame.Rect(dbutton_plus_x, dbutton_plus_y, button_width, button_height)
    dbutton_minus_rect = pygame.Rect(dbutton_minus_x, dbutton_minus_y, button_width, button_height)
    ...
    pygame.draw.rect(screen, zaklad_button_color, dbutton_plus_rect)
    pygame.draw.rect(screen, zaklad_button_color, dbutton_minus_rect)
    pygame.draw.rect(screen, get_sbutton_color(), sbutton_rect)
    ...
    screen.blit(plus_text, (dbutton_plus_x + 45, dbutton_plus_y + 8))
    screen.blit(minus_text, (dbutton_minus_x + 45, dbutton_minus_y + 8))
    screen.blit(dvalue_text, (drvalue_x + 10, drvalue_y + 8))
    screen.blit(sbutton_text, (sbutton_x + 10, sbutton_y + 8))
